        ...
        <div class="container-xxl">
            <div class="container">
                <div class="text-center wow fadeInUp" data-wow-delay="0.1s">
                    <h1 class="mb-5 text-uppercase">{{$article->title}}</h1>
                </div>
    ...
    <div class="col-md-8 py-3 mb-2">
        <div class="container-xxl">
            <div class="row">
                <div class="col-md-4 col-sm-6 text-start fw-bold">
                    <button class="btn btn-transparent">
                        <i>Created
                            {{$article->created_at->diffForHumans()}}</i>
                    </button>
                </div>
                <div class="col-md-5 col-sm-6 row outline-secondary">
                    <div class="col-4">
                        <button class="btn btn-transparent">
                            {{$article->views->count()}} <i class="fa fa-eye"></i>
                        </button>
                    </div>
                    <div class="col-4">
                        <button class="btn btn-transparent">
                    ...
                    <div class="col-4">
                        <form action="{{route('alike.store', ['post_id' => $article->id])}}" method="post">
                            @csrf
                            <button type="submit" class="btn btn-transparent">{{$article->likes->count()}} <i
            ...
            @foreach ($article->comments as $comment)
                ...
                <hr>
            @endforeach
            <form action="{{route('acomment.store', ['post_id' => $article->id])}}" method="post">
                @csrf
